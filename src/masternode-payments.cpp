...
//    CAmount nMaxMintValue = GetBlockValue(nHeight);
    if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything
        ...
        if (nHeight % GetBudgetPaymentCycleBlocks() < 100) {
            ...
            if (nHeight <= REV_BLOCK && nHeight % PHZ_BLOCK != 0 && nMinted > nExpectedValue) {
                if (fDebug)
                    LogPrint("masternode","Reverse Block #1: Reward is not pays actual=%s > limit=%s Blocks=%s\n",
                ...
                if (fDebug)
                    LogPrint("masternode","Reverse Block #1: Reward pays actual=%s < limit=%s Blocks=%s\n",
                            FormatMoney(nMinted), FormatMoney(nExpectedValue), nHeight);
            }
        }
    ...
    //are these blocks even enabled
        if (!IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS)) {
            if(nHeight <= REV_BLOCK && nMinted >= nExpectedValue) {
                if (fDebug)
                LogPrint("masternode","Reverse Block #2: Reward pays actual=%s >= limit=%s Blocks=%s\n",
                            FormatMoney(nMinted), FormatMoney(nExpectedValue), nHeight);
            }
            else if(nHeight > REV_BLOCK && nMinted <= nExpectedValue) {
                if (fDebug)
        ...
        if (budget.IsBudgetPaymentBlock(nHeight)) {
            ...
            if (nHeight <= REV_BLOCK && nHeight % PHZ_BLOCK != 0 && nMinted > nExpectedValue) {
                if (fDebug)
                    LogPrint("masternode","Reverse Block #3: Reward is not pays actual=%s > limit=%s Blocks=%s\n",
                            FormatMoney(nMinted), FormatMoney(nExpectedValue), nHeight);
                return false;
            }
            else if (nHeight > REV_BLOCK && nMinted > nExpectedValue) {
                ...
                if (fDebug)
                    LogPrint("masternode","Reverse Block #3: Reward pays actual=%s < limit=%s Blocks=%s\n",
                            FormatMoney(nMinted), FormatMoney(nExpectedValue), nHeight);
            }
        }
    }
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZLRMStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZLRMStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZLRMStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZLRMStake);
